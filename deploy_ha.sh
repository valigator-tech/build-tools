...
if [[ "$CLUSTER" == "list" ]]; then
  echo "Available HA packages for deployment:"
  echo "======================================"
  echo
  ...
  if [[ -d "$artifact_dir" ]]; then
    ...
    while IFS= read -r -d '' file; do
      ...
      # Extract tag from filename (e.g., ha-v0.1.0.tar.gz -> v0.1.0)
    ...
    if [[ ${#packages[@]} -gt 0 ]]; then
      ...
      echo "No packages found. Run build_ha.sh first to create packages."
    fi
  else
    echo "No packages found. Run build_ha.sh first to create packages."
  fi
...
if [[ ! -f "$TARBALL" ]]; then
  ...
  echo "Did you run build_ha.sh for tag '$TAG'?" >&2
  exit 1
fi
